...
namespace ffm
{
    ...
    namespace
    {
        ...
        inline ffm_float wTx(ffm_node* begin, ffm_node* end, ffm_float r,
                             ffm_model& model, ffm_float kappa = 0,
                             ...
                             bool do_update = false)
        {
            ...
            for(ffm_node* N1 = begin; N1 != end; N1++)
            {
                ...
                for(ffm_node* N2 = N1 + 1; N2 != end; N2++)
                {
                    ...
                    ffm_float* w1_base =
                        model.W + (ffm_long)j1 * align1 + f2 * align0;
                    ffm_float* w2_base =
                        model.W + (ffm_long)j2 * align1 + f1 * align0;
                    ...
                    if(do_update)
                    {
                        ...
                        for(ffm_int d = 0; d < align0; d += kALIGN * 2)
                        {
                            ...
                            XMMwg1 =
                                _mm_add_ps(XMMwg1, _mm_mul_ps(XMMg1, XMMg1));
                            XMMwg2 =
                                _mm_add_ps(XMMwg2, _mm_mul_ps(XMMg2, XMMg2));
...
#else
            int status =
                posix_memalign(&ptr, kALIGNByte, size * sizeof(ffm_float));
            if(status != 0)
                throw bad_alloc();
#endif
        ...
        struct problem_on_disk
        {
            ...
            problem_on_disk(string path)
            {
                ...
                if(f.good())
                {
                    f.read(reinterpret_cast<char*>(&meta),
                           sizeof(disk_problem_meta));
                    f.seekg(meta.B_pos);
        ...
        uint64_t hashfile(string txt_path, bool one_block = false)
        {
            ...
            for(ffm_long pos = 0; pos < end;)
            {
                ...
                {
                    uint64_t x =
                        *reinterpret_cast<uint64_t*>(buffer.data() + i);
        ...
        bool check_same_txt_bin(string txt_path, string bin_path)
        {
            ...
            f_bin.read(reinterpret_cast<char*>(&meta),
                       sizeof(disk_problem_meta));
    ...
    void ffm_read_problem_to_disk(string txt_path, string bin_path)
    {
        ...
        cout << "First check if the text file has already been converted to "
                "binary format "
             << flush;
        ...
        {
            cout << "Binary file NOT found. Convert text file to binary file "
                 << flush;
            txt2bin(txt_path, bin_path);
            cout << "(" << fixed << setprecision(1) << timer.toc()
                 << " seconds)" << endl;
        }
    }
...
#pragma omp parallel for schedule(static) reduction(+ : loss)
    ...
    void ffm_save_model(ffm_model& model, string path)
    {
        ...
        f_out.write(reinterpret_cast<char*>(&model.normalization),
                    sizeof(bool));

        ffm_long w_size = get_w_size(model);
        // f_out.write(reinterpret_cast<char*>(model.W), sizeof(ffm_float) *
        // w_size); Need to write chunk by chunk because some compiler use int32
        // and will overflow when w_size * 4 > MAX_INT
    ...
    ffm_model ffm_load_model(string path)
    {
        ...
        // f_in.read(reinterpret_cast<char*>(model.W), sizeof(ffm_float) *
        // w_size); Need to write chunk by chunk because some compiler use int32
        // and will overflow when w_size * 4 > MAX_INT
